...
function csrfPlugin(server, options, next) {
  ...
  server.ext("onPreAuth", (request, reply) => {
    ...
    function verifyAndCreateToken() {
      ...
      return Promise.all([csrf.verify(headerPayload), csrf.verify(cookiePayload)])
        .spread((headerToken, cookieToken) => {
          if (
            headerToken.uuid === cookieToken.uuid &&
            headerToken.type === "header" &&
            cookieToken.type === "cookie"
          ) {
            return createToken();
          }
    ...
    function shouldSkip() {
      return (
        request.route.settings.plugins["electrode-csrf-jwt"] &&
