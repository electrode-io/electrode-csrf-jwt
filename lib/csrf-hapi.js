...
function csrfPlugin(server, options, next) {
  ...
  const createToken = request => {
    ...
    if (plugin && !plugin.header) {
      const tokens = csrf.create();
      plugin.header = tokens.header;
      plugin.cookie = tokens.cookie;
    }
  ...
  server.ext("onPreResponse", (request, reply) => {
    const plugin = createToken(request);
    ...
    if (plugin) {
      ...
      reply.state(csrf.cookieName, plugin.cookie, cookieConfig);

      headers[csrf.headerName] = plugin.header;
    }
